...
export default function ProgrammingLab() {
  ...
  const [language,setLanguage]=useState(initialLanguage); const [source,setSource]=useState(SAMPLES[initialLanguage]||""); const [stdin,setStdin]=useState("5 7"); const [stdout,setStdout]=useState(""); const [stderr,setStderr]=useState(""); const [status,setStatus]=useState("idle"); const [capabilities,setCapabilities]=useState([]); const [serviceError,setServiceError]=useState(""); const controllerRef=useRef(null);
  const config=getCompilerConfig();
  useEffect(()=>{if(!config.configured)return;const c=new AbortController();getCompilerCapabilities({signal:c.signal}).then(r=>setCapabilities(r.languages)).catch(e=>setServiceError(e.message));return()=>c.abort();},[]);
  ...
  const run=async()=>{if(!supported){setStatus("unsupported");setStderr(`${selected?.name || "This language"} execution is coming soon. Secure sandbox infrastructure must be connected before public execution can be enabled.`);return;}setStatus("loading");setStdout("");setStderr("");const controller=new AbortController();controllerRef.current=controller;try{const result=selected.browser?await runJavascriptWorker(source,stdin,{timeoutMs:3000,signal:controller.signal}):await executeRemoteCode({language,source,stdin,timeoutMs:5000,signal:controller.signal});setStdout(result.stdout||"");setStderr(result.stderr||"");setStatus(result.exitCode===0?"success":"error");}catch(e){if(e.name==="AbortError"){setStderr("Execution stopped by user.");}else setStderr(e.message||"Execution failed.");setStatus("error");}finally{controllerRef.current=null;}};
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-navy-100 bg-white p-4 dark:border-navy-800 dark:bg-navy-900"><select className="mz-input max-w-xs" value={language} onChange={(e)=>changeLanguage(e.target.value)}><option value="web">HTML/CSS/JS Playground</option>{COMPILER_LANGUAGES.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select><span className={`rounded-full px-3 py-1 text-xs font-bold ${supported?"bg-emerald-100 text-emerald-800":"bg-amber-100 text-amber-800"}`}>{selected?.browser?"Browser supported":backendSupported?"Backend connected":"Coming soon"}</span><span className="ml-auto flex items-center gap-1.5 text-xs text-navy-400"><ShieldCheck className="h-4 w-4"/>{selected?.browser?"Isolated worker · network blocked":"Remote execution must be isolated"}</span></div>
    {!supported?<div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200"><div className="flex gap-2"><Server className="mt-0.5 h-5 w-5 shrink-0"/><div><strong>{selected?.name} execution is coming soon 🚀</strong><p className="mt-1">We are enabling this carefully through an isolated compiler service so your code can run with proper CPU, memory, timeout and filesystem limits. JavaScript and the HTML/CSS/JS playground are available now.</p>{serviceError?<p className="mt-2 text-xs opacity-80">The secure execution service is not currently reachable.</p>:null}</div></div></div>:null}
