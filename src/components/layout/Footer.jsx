  ...
  return (
    <footer className="mt-16 border-t border-navy-100 bg-white dark:border-navy-800 dark:bg-navy-950">
      <div className="mz-section grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div><h4 className="text-sm font-semibold">Tools</h4><ul className="mt-4 space-y-2.5">{categories.slice(0,5).map(c=><li key={c.slug}><Link to={`/categories/${c.slug}`} className="text-sm text-navy-500 hover:text-brand-600 dark:text-navy-400">{c.name}</Link></li>)}</ul></div>
