...
const QUEUE_KEY = "mz-feedback-queue-v1";
const CATEGORIES = ["Bug Report", "Feature Request", "Tool Suggestion", "UI/UX Feedback", "General Feedback"];

function readQueue() { try { const value = JSON.parse(localStorage.getItem(QUEUE_KEY) || "[]"); return Array.isArray(value) ? value : []; } catch { return []; } }
function queueFeedback(payload) { try { const next = [...readQueue(), payload].slice(-25); localStorage.setItem(QUEUE_KEY, JSON.stringify(next)); return next.length; } catch { return 0; } }
function deviceLabel() { const width = window.innerWidth; return width < 640 ? "mobile" : width < 1024 ? "tablet" : "desktop"; }
...
export default function FeedbackDialog({ open, onClose }) {
  ...
  useEffect(() => {
    const flush = async () => {
      ...
      if (remaining.length < queued.length) notify(`${queued.length - remaining.length} saved feedback item${queued.length - remaining.length === 1 ? "" : "s"} sent.`, { type:"success", title:"Feedback synced" });
    };
    flush();
  ...
  async function submit(event) {
    ...
    const payload = { id: crypto.randomUUID?.() || String(Date.now()), name:name.trim(), email:email.trim(), category, rating: rating || "", message:clean, page:currentPage, device:deviceLabel(), createdAt:new Date().toISOString() };
    ...
    if (!isLocal) {
      try {
        ...
        setStatus("queued"); setNotice(count ? "We couldn't reach the feedback service, so your feedback was saved safely on this device. It has not been sent yet." : "Feedback could not be sent or saved on this device. Please try again later."); return;
      }
    }
    const count = queueFeedback(payload);
    setStatus("queued"); setNotice(count ? "Local development mode: your feedback was saved on this device only. Netlify Forms will receive submissions after production deployment." : "Local feedback storage is unavailable in this browser.");
  }
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-navy-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="feedback-title" onMouseDown={(e)=>{if(e.target===e.currentTarget)onClose?.()}}>
    <div className="w-full max-w-xl rounded-3xl border border-white/60 bg-white p-5 shadow-2xl dark:border-navy-700 dark:bg-navy-900 sm:p-7">
      ...
      <form className="mt-5 space-y-4" onSubmit={submit}>
        ...
        <div className="rounded-xl bg-navy-50 px-3 py-2 text-xs text-navy-500 dark:bg-navy-950 dark:text-navy-400">Page: <strong>{currentPage}</strong> · Device: <strong>{deviceLabel()}</strong></div>
